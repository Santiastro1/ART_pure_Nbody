...
c       character sntime*5  !para simulaciones con HART
       ...
       character head1*6
       character head2*7
       dimension xpart(80000000,11),ypart(80000000,11),zpart(80000000,11
     +),vxpart(80000000,11),vypart(80000000,11),vzpart(80000000,11),nums
...
cc       Narg = IARGC ()
cc       call getarg(1,sntime)
cc       write(*,*)'num args = ', narg, sntime
c       sntime='0.6920'
c      OPEN(17,FILE=FileASCII,STATUS='UNKNOWN')
      open(3,file=head1//sntime//tail,form='unformatted',status='unknown
     &')
cc      open(3,file='PMcrd_ini.DAT',form='unformatted',status='unknown
cc     &')
 ...
 10   npaget    = nrow**2    ! # of particles in a page
      ...
      yn=float(ngrid)
      write(*,*)'Nspec=',Nspecies
c      nrowc=1024
      open ( 1 , file =head2//sntime//tail, access = 'direct',
     &           status = 'unknown', recl = nacces      )
cc       open ( 1 , file ='PMcrs0_ini.DAT', access = 'direct',
cc     &           status = 'unknown', recl = nacces      )
...
c      read(*,*)nopti
      ...
      write(*,*)'Número estrelles disco',diskpart
      write(*,*)'Número de estrellas bulbo',bulgepart
      OPEN(17,FILE=head2//sntime//tailtip,STATUS='UNKNOWN')
      write(17,*)lspecies(nspecies),0,int((diskpart+bulgepart)/scrip)
      write(17,*)3
      write(17,*)0
       ncont=0      
      do j=1,lspecies(1)-diskpart-bulgepart,scrip
...
c 1223 continue
      write(*,*)ncont
      do i=1,diskpart+bulgepart,scrip
         write(17,*)pdmass/(3.17e15)
         ncont=ncont+1
      enddo
         write(*,*)ncont
...
c        write(*,*)ncont
      ...
      do i=1+diskpart+bulgepart,ip,scrip
...
c           xmin =MIN(xmin,X(i),Y(i),Z(i))
     ...
     &pdmass,1
      enddo
      do i=diskpart+1,diskpart+bulgepart
         x(i)=(x(i)*1000.-xmean)/hubble
         y(i)=(y(i)*1000.-ymean)/hubble
         z(i)=(z(i)*1000.-zmean)/hubble
         write(17,101)x(i),y(i),z(i),vxx(i),vyy(i),vzz(i),
     &pdmass,3
      ...
      do i=diskpart+bulgepart+1,lspecies(1)
         x(i)=(x(i)*1000.-xmean)/hubble
         y(i)=(y(i)*1000.-ymean)/hubble
         z(i)=(z(i)*1000.-zmean)/hubble
         write(17,101)x(i),y(i),z(i),vxx(i),vyy(i),vzz(i),
